...
def displayDyn(screen,racquet)
  ...
  #this is what I have tried to display racquet
  #(1..SCREEN_X - 1).each do |col|
  #    screen[SCREEN_Y - 1][col] = SC_H
  #end
  #((racquet - RACQUET_SIZE/2 + 1)..(racquet + RACQUET_SIZE/2 + 1 )).each do |rac|
  #    print "\e[#{SCREEN_Y + 1};#{rac}H="
  #end
  ...
  if racquet >= 1 && racquet < SCREEN_X
		# gets rid of the old racquet position
    (2...SCREEN_X).each do |col| 
      print "\e[#{SCREEN_Y};#{col}H-"
    end
		# shows the new position
    (1...SCREEN_X).each do |col| 
      if col >= racquet && col < racquet + 4
        print "\e[#{SCREEN_Y};#{col}H="
      end
    end
  end   
...
def displayBoundaries(screen)
  ...
  print "\e[2J\e[#{1};#{1}H"  #change from puts to print
  screen.each do |row|
      row.each do |col|
          print col
      end
